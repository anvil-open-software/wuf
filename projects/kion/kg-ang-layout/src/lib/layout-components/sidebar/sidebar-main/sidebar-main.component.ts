...
export class KgSidebarMainComponent implements OnInit, OnDestroy {
    ...
    toggleSidebar($event: any) {
        let isExpanded = this.getIsExpanded();
    ...
    showSidebar() {
        ...
        let transitionInCss: string = this.sidebarWrap.nativeElement.style.transitionDuration || '0.5s';
        let multiplier: number = transitionInCss.indexOf('ms') > 0 ? 1 : 1000;
        let transitionTimeMatch: any = parseFloat(transitionInCss);
        let transitionTime = parseFloat(transitionTimeMatch) * multiplier;
    ...
    restoreSidebarStyles(newWidth?: number, turnOffTransition?: boolean) {
        ...
        let sidebarTransition = this.kgSidebarService.sidebarIsResizing || turnOffTransition ? 'none!important' : this._storedTransition;
        let sidebarStyles = `transition:${sidebarTransition}`;
    ...
    sidebarResizing($event: any) {
        let delta = this.kgSidebarService.mouseOutOfWindow ? 0 : $event.movementX;
